...
description: Add a compute backend or GPU kernel to Piramid. Use when implementing a DistanceKernels backend, writing CUDA kernels, touching crates/compute or crates/gpu, or working on vector memory layout and device transfer.
---
...
`crates/gpu` owns the **device**: contexts, buffers, streams, module loading. No math semantics.
`crates/compute` owns the **math**: what cosine means, which backend runs it.

Both are leaf crates — they depend on nothing else in the workspace. Do not add a dependency to
either; `scripts/check-deps.sh` will reject it, and the reason is that `crates/inference` needs a
device too and must not reach through retrieval math to get one.

Vendor SDK types (`cudarc`) appear only in `crates/gpu/src/backends/`. If `cudarc::` shows up
anywhere else, the abstraction has leaked.

## Adding a CPU backend

One file in `crates/compute/src/backends/`, one arm in `for_mode` in `backends/mod.rs`. Nothing
...
2. Kernel source in `crates/gpu/src/kernels/<family>.cu`, typed launch wrapper beside it in
   `<family>.rs`. The wrapper owns launch geometry and argument binding, not device lifetime.
3. Wire it in `crates/compute/src/backends/cuda.rs` by overriding the batch methods only. Leave
...
- `#[allow(unsafe_code)]` outside `crates/gpu` — the security workflow fails on a new site.
