...
| `compose.prod.yml` | Overlay that swaps in GHCR images. `just prod-up`. |
...
just logs          # follow
just up ollama     # add local embeddings
just down
```

## Production

Images are published to `ghcr.io/ashworks1706/piramid` by `.github/workflows/cd.yml` on every push
to `main`, tagged with both the commit SHA and `main`.
...
`/data`. Health is `GET /api/health` and readiness is `GET /api/readyz`; `GET /metrics` is the
Prometheus endpoint.
